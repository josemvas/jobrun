...
def queuejob(sysconf, jobconf, options, scheduler, inputfile):
    ...
    version = c.null.join(c.lower() for c in options.version if c.isalnum())

    for ext in jobconf.inputfiles:
        try: jobconf.fileexts[ext]
        except KeyError: post('La extensión de archivo de salida', ext, 'no fue definida', kind=rc.cfgerr)
    ...
    iosuffix = { ext : version + c.dot + ext for ext in jobconf.fileexts }

    if not jobname:
        for ext in jobconf.inputfiles:
            if filename.endswith(c.dot + ext):
                jobname = filename[:-len(c.dot + ext)]
                ionames = { ext : ext for ext in jobconf.fileexts }
                break

    if not jobname:
        post('La extensión', path.splitext(filename)[1], 'no es de un archivo de entrada de', jobconf.title, kind=rc.joberr)
        return
    ...
    if 'filecheck' in jobconf:
        if not parse_boolexpr(jobconf.filecheck, filebool):
            post('No existen algunos de los archivos de entrada requeridos', kind=rc.joberr)
            return

    if 'fileclash' in jobconf:
        if parse_boolexpr(jobconf.fileclash, filebool):
            post('Hay un conflicto entre algunos de los archivos de entrada', kind=rc.joberr)
            return
    ...
    if sysconf.storage == 'pooled':
         ...
         post(sysconf.storage + ' no es un tipo de almacenamiento soportado por este script', kind=rc.cfgerr)
    ...
    if jobconf.runtype == 'serial':
        ...
        if jobconf.mpiwrapper is True:
            jobcommand = scheduler.mpiwrapper[jobconf.runtype] + c.ws + jobcommand
    else: post('El tipo de paralelización ' + jobconf.runtype + ' no es válido', kind=rc.cfgerr)
    ...
    if 'stdin' in jobconf:
        try: redirections.append('0<' + c.ws + jobconf.fileexts[jobconf.stdin])
        except KeyError: post('El nombre de archivo "' + jobconf.stdin + '" en el tag <stdin> no fue definido.', kind=rc.cfgerr)
    if 'stdout' in jobconf:
        try: redirections.append('1>' + c.ws + jobconf.fileexts[jobconf.stdout])
        except KeyError: post('El nombre de archivo "' + jobconf.stdout + '" en el tag <stdout> no fue definido.', kind=rc.cfgerr)
    if 'stderr' in jobconf:
        try: redirections.append('2>' + c.ws + jobconf.fileexts[jobconf.stderr])
        except KeyError: post('El nombre de archivo "' + jobconf.stderr + '" en el tag <stderr> no fue definido.', kind=rc.cfgerr)
    ...
    if 'optionargs' in jobconf:
        for opt in jobconf.optionargs:
            ...
            arguments.append('-' + opt + c.ws + jobconf.fileexts[ext])

    jobdir = pathjoin(outputdir, [c.null, jobname, version])
        ...
        if path.isdir(outputdir):
                ...
                try:
                    ...
                    if jobstate in scheduler.jobstates:
                        post('El trabajo', quote(jobname), 'no se envió porque', scheduler.jobstates[jobstate], \
                            '(job {})'.format(lastjob), kind=rc.joberr)
                        return
            ...
            if not set(listdir(outputdir)).isdisjoint([ pathjoin([jobname, iosuffix[ext]]) for ext in jobconf.outputfiles ]):
                if options.defaultanswer is None:
                    options.defaultanswer = prompt('Si corre este cálculo los archivos de salida existentes en el directorio', outputdir,'serán sobreescritos, ¿desea continuar de todas formas (si/no)?', kind=ic.yn)
                if options.defaultanswer is False:
                    post('El trabajo', quote(jobname), 'no se envió por solicitud del usuario', kind=rc.joberr)
                    return
            for ext in jobconf.inputfiles + jobconf.outputfiles:
                remove(pathjoin(outputdir, [jobname, iosuffix[ext]]))
        elif path.exists(outputdir):
            post('No se puede crear la carpeta', outputdir, 'porque hay un archivo con ese mismo nombre', kind=rc.joberr)
            return
        else:
            mkdir(outputdir)
            mkdir(jobdir)
    ...
    try:
        ...
        fh.write(textform([textform(line, end=c.null) for line in jobcontrol], sep=c.nl))
        fh.write(textform(environment, sep=c.nl))
        fh.write(textform('for ip in ${iplist[*]}; do'))
        fh.write(textform('ssh', master, 'ssh $ip mkdir -m 700', quote(quote('$workdir')), indent=4))
        fh.write(textform([textform(line, indent=4, end=c.null) for line in importfiles], sep=c.nl))
        fh.write(textform('done'))
        fh.write(textform('cd', quote('$workdir')))
        fh.write(textform([i.repr for i in jobconf.prescript if 'iff' not in i or filebool[i.iff]], sep=c.nl)) \
            if 'prescript' in jobconf else None
        fh.write(textform(jobcommand, arguments, redirections))
        fh.write(textform([textform(line, end=c.null) for line in exportfiles], sep=c.nl))
        fh.write(textform('for ip in ${iplist[*]}; do'))
    ...
    try:
        ...
        post('El sistema de colas rechazó el trabajo', quote(jobname), 'con el mensaje', quote(e.args[0]), kind=rc.joberr)
    else:
        post('El trabajo', quote(jobname), 'se correrá en', str(options.ncpu), 'núcleo(s) de CPU (job', jobid + ')', kind=rc.sucess)

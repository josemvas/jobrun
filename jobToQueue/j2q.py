...
from jobToQueue import j2q
j2q.run('{hostspecs}', '{jobspecs}')
'''
...
def setup():
    ...
    host = prompt('Seleccione la opción con la arquitectura más adecuada', kind=ic.radio, choices=listdir(platformdir))

    if not path.isfile(pathjoin(platformdir, host, 'hostspecs.xml')):
        post('El archivo de configuración de la plataforma', quote(host), 'no existe', kind=rc.cfgerr)

    if path.isfile(pathjoin(specdir, 'hostspecs.xml')):
        if prompt('El sistema ya está configurado, ¿quiere reinstalar la configuración por defecto (si/no)?', kind=ic.yn):
    ...
    for package in listdir(pathjoin(platformdir, host)):
        if path.isdir(pathjoin(platformdir, host, package)):
            try:
                ...
                post('El archivo', pathjoin(genericdir, package, 'jobspecs.xml'), 'no tiene un título', kind=rc.cfgerr)
    ...
    if not packagelist:
        post('No hay paquetes configurados para este host', kind=rc.warning)
        return

    enabled = prompt('Marque/desmarque los paquetes para los que desea escribir/borrar la configuración por defecto/actual', kind=ic.check, choices=packagelist, default=configured)
    ...
    binpath = path.expanduser(prompt('Especifique la ruta donde se instalarán los enlaces de los paquetes configurados (ENTER para omitir)', kind=ic.path))

...
if 'filecheck' in jobconf:
    if not parsebool(jobconf.filecheck, filebools):
        messages.failure('No existen algunos de los archivos de entrada requeridos')
        raise RuntimeError()
        ...
        if os.path.isdir(jobdir):
            try:
                ...
                if jobstate in queueconf.jobstates:
                    messages.failure('El trabajo', q(jobname), 'no se envió porque', queueconf.jobstates[jobstate], '(jobid {0})'.format(lastjob))
                    raise RuntimeError()

...
def configure():
    ...
    jobenv = BunchDict(scheduler.jobenv)
    ...
    parser.add_argument('-l', '--list', dest='lsoptions', action='store_true', help='Imprimir las versiones de los programas y parámetros disponibles.')
    ...
    parser.add_argument('-R', '--remote', metavar='HOSTNAME', type=str, dest='remote', help='Ejecutar el trabajo remotamente en HOSTNAME.')
    ...
    parser.add_argument('--cwdir', metavar='CWDIR', type=str, dest='cwdir', help='Cambiar el directorio de trabajo.')
    parser.add_argument('--outdir', metavar='OUTDIR', type=str, dest='outdir', help='Cambiar el directorio de salida.')
    parser.add_argument('--scrdir', metavar='SCRATCHDIR', type=str, dest='scrdir', help='Cambiar el directorio de escritura.')
    parser.add_argument('--node', metavar='NODENAME', type=str, dest='execnode', help='Solicitar un nodo específico de ejecución.')
    ...
    optconf.update(vars(parsed))
    
    if optconf.lsoptions:
        if jobconf.versions:
    ...
    if optconf.cwdir:
        if isabspath(optconf.cwdir):
            optconf.cwdir = normalpath(optconf.cwdir)
        else: 
            optconf.cwdir = normalpath(jobconf.rootdir, optconf.cwdir)
    ...
    control.append(jobstr.label(jobconf.packagename))
    control.append(jobstr.queue(optconf.queue))
    control.append(jobstr.stdout(optconf.scrdir))
    control.append(jobstr.stderr(optconf.scrdir))
    
    if optconf.execnode:
        control.append(jobstr.hosts(optconf.execnode))
    
    #TODO: MPI support for Slurm
    if jobconf.parallelization:
        if jobconf.parallelization.lower() == 'none':
            control.append(jobstr.nhost(optconf.nhost))
        elif jobconf.parallelization.lower() == 'openmp':
            control.append(jobstr.ncore(optconf.ncore))
            control.append(jobstr.nhost(optconf.nhost))
            ...
            if jobconf.usempilauncher:
                command.append(mpilauncher[jobconf.parallelization])
        else:
    ...
    environment.extend('='.join(i) for i in jobenv.items())
    ...
    environment.append("workdir=" + q(pathjoin(optconf.scrdir, jobenv.jobid)))
    ...
    for key in jobconf.optionargs:
        ...
        command.append(wordjoin('-' + key, jobconf.filekeys[jobconf.optionargs[key]]))
    ...
    for item in jobconf.positionargs:
        ...
        command.append('@' + p('|'.join(jobconf.filekeys[i] for i in item.split('|'))))
    
    if 'stdin' in jobconf:
        try: command.append('0<' + ' ' + jobconf.filekeys[jobconf.stdin])
        except KeyError: messages.cfgerror('El nombre de archivo "' + jobconf.stdin + '" en el tag <stdin> no fue definido.')
    if 'stdout' in jobconf:
        try: command.append('1>' + ' ' + jobconf.filekeys[jobconf.stdout])
    ...
    if 'stderr' in jobconf:
        try: command.append('2>' + ' ' + jobconf.filekeys[jobconf.stderr])

...
def setup(relpath=False):
    ...
    copyfile(path.join(sourcedir, 'execs','jobsync'), path.join(bindir, 'jobsync'))
    ...
    for dirname in listdir(specdir):
        ...
        with open(path.join(sourcedir, 'execs', 'launcher'), 'r') as fr, open(path.join(bindir, dirname), 'w') as fw:
